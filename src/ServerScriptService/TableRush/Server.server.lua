...
local ROOM_TEMPLATES = {
    ...
    {Id="room_21", Name="Glass Hall", Theme="Trap", Difficulty=4, Atmosphere={TableColor={58,38,32}, Glow={230,90,62}, Material="Cobblestone", Sound="rbxassetid://9113420778"}, Props={"Spikes","PressurePlate"}, SearchOutcomes={{Id="bad", Weight=45, Text="Something hears you.", Effect="Threat"},{Id="trap", Weight=25, Text="Search hurts you.", Effect="Damage"},{Id="good", Weight=15, Text="Find a clue.", Effect="Light"},{Id="nothing", Weight=15, Text="Nothing answers.", Effect="Nothing"}}, SchemeOutcomes={{Id="backfire", Weight=45, Text="This room punishes greed.", Effect="Damage"},{Id="payoff", Weight=25, Text="Big pot risk.", Effect="Pot"},{Id="curse", Weight=20, Text="Threat surges.", Effect="Threat"},{Id="shortcut", Weight=10, Text="Open a route clue.", Effect="Reveal"}}, Tiles={{Id="start", Label="Entry", Kind="Start", X=0, Y=0, Neighbors={"enemy","cache"}},{Id="enemy", Label="Needle Fiend", Kind="Enemy", X=1, Y=0, Neighbors={"start","exit"}, Enemy="Needle Fiend", HP=5},{Id="cache", Label="Side Cache", Kind="Treasure", X=0, Y=1, Neighbors={"start","exit"}},{Id="exit", Label="Exit", Kind="Exit", X=2, Y=0, Neighbors={"enemy","cache"}}}, Doors={{Id="safe", Label="Safe Door", Hint="Lower danger route.", Bias=1},{Id="greed", Label="Greed Door", Hint="More loot, more risk.", Bias=2},{Id="deep", Label="Deep Door", Hint="Harder next room.", Bias=3}}},
    ...
    {Id="room_29", Name="Needle Gauntlet", Theme="Trap", Difficulty=5, Atmosphere={TableColor={58,38,32}, Glow={230,90,62}, Material="Cobblestone", Sound="rbxassetid://9113420778"}, Props={"Spikes","PressurePlate"}, SearchOutcomes={{Id="bad", Weight=45, Text="Something hears you.", Effect="Threat"},{Id="trap", Weight=25, Text="Search hurts you.", Effect="Damage"},{Id="good", Weight=15, Text="Find a clue.", Effect="Light"},{Id="nothing", Weight=15, Text="Nothing answers.", Effect="Nothing"}}, SchemeOutcomes={{Id="backfire", Weight=45, Text="This room punishes greed.", Effect="Damage"},{Id="payoff", Weight=25, Text="Big pot risk.", Effect="Pot"},{Id="curse", Weight=20, Text="Threat surges.", Effect="Threat"},{Id="shortcut", Weight=10, Text="Open a route clue.", Effect="Reveal"}}, Tiles={{Id="start", Label="Entry", Kind="Start", X=0, Y=0, Neighbors={"enemy","cache"}},{Id="enemy", Label="Needle Fiend", Kind="Enemy", X=1, Y=0, Neighbors={"start","exit"}, Enemy="Needle Fiend", HP=6},{Id="cache", Label="Side Cache", Kind="Treasure", X=0, Y=1, Neighbors={"start","exit"}},{Id="exit", Label="Exit", Kind="Exit", X=2, Y=0, Neighbors={"enemy","cache"}}}, Doors={{Id="deep", Label="Deep Door", Hint="Hard route.", Bias=3},{Id="vault", Label="Vault Door", Hint="Greed route.", Bias=4},{Id="secret", Label="Strange Door", Hint="Rare special route.", Bias=2}}},
...
local function pickRoomTemplate(state, entryDoor)
    ...
    if idx<=1 then return ROOM_TEMPLATES[1] end
    if idx >= (state.RoomsToWin or 15) then return ROOM_TEMPLATES[39] end
    if entryDoor=="secret" then return ROOM_TEMPLATES[40] end
    local target=math.clamp(math.floor(idx/3)+1,2,7)
    local weighted={}
    for _,r in ipairs(ROOM_TEMPLATES) do
        if r.Theme~="Tutorial" and r.Theme~="Boss" and r.Theme~="Secret" then
            local d=r.Difficulty or 1; local w=1
            if d==target then w += 8 end; if d==target+1 then w += 5 end; if d==target-1 then w += 3 end
            if idx>=10 and d>=5 then w += 8 end
            if entryDoor=="deep" and d>=target then w += 8 end
            if (entryDoor=="greed" or entryDoor=="vault") and (r.Theme=="Greed" or r.Theme=="Treasure" or r.Theme=="Ambush") then w += 9 end
            for _=1,math.max(1,w) do table.insert(weighted,r) end
        end
    end
...
local function currentTileHasInteract(state)
    ...
    if tile.Kind=="Discovery" and not tile.Cleared then return true, "Use Discovery" end
    if tile.Kind=="Trap" and not tile.Cleared then return true, "Disarm Trap" end
    if tile.Kind=="Exit" then return true, "Use Exit Door" end
    return false, "Nothing to use here"
end
...
local function rebuildCommandList(state)
    ...
    local anyMonster=hasAnyLiveRevealedMonster(state)
    local searched=currentTileSearchSpent(state)
    local hasInteract, interactLabel=currentTileHasInteract(state)

    table.insert(actions,{Key=Constants.ACTIONS.Step,Title=liveMonster and "Run Away" or "Move Figure",Icon="↔",Body=liveMonster and "WASD/click a revealed connected room. Unknown rooms are blocked." or "WASD/click a connected room. E interacts when something is usable.",Tag=liveMonster and "ESCAPE" or "MOVE"})
    if liveMonster then
        table.insert(actions,{Key=Constants.ACTIONS.Strike,Title="Fight Monster",Icon="⚔",Body="Attack the monster on this tile. Doors stay locked while monsters live.",Tag="COMBAT"})
        table.insert(actions,{Key=Constants.ACTIONS.Shield,Title="Brace",Icon="⬟",Body="Brace against the monster intent and reduce Threat.",Tag="DEFEND"})
    else
        if not searched then
            table.insert(actions,{Key=Constants.ACTIONS.Search,Title="Search This Tile",Icon="◆",Body="Once per tile for the whole team. Can reveal clues, objects, safer routes, or risk.",Tag="CLUE"})
        end
        if hasInteract then
            table.insert(actions,{Key=Constants.ACTIONS.Interact,Title=interactLabel,Icon="✦",Body=anyMonster and "Locked until revealed monsters are defeated." or "Use the physical object/state on this tile.",Tag="USE"})
        end
        table.insert(actions,{Key=Constants.ACTIONS.Shield,Title="Brace",Icon="⬟",Body="Stabilize. Reduce Threat and prepare for enemy intent.",Tag="SAFE"})
    end
...
local function syncLegacyFields(state)
    ...
    for _,tile in ipairs(state.Board.Tiles or {}) do
        if tile.Kind=="Enemy" and not tile.Cleared and tile.Revealed then
            table.insert(state.Enemies,{Id=tile.Id,Name=tile.Enemy,Zone=tile.Label,HP=tile.HP or 1,MaxHP=tile.MaxHP or tile.HP or 1,IntentIcon="!",Intent="Doors are locked while this monster lives.",Kind=tile.Enemy})
        end
    end
...
local function makeDungeonState(player) local room=makeRoomState(nil,1,"start"); local state={Phase="Your Turn",RoomIndex=1,RoomsToWin=15,BonusRooms=0,MaxRooms=18,Pot=250,Threat=2,CurrentRoom={Key="Room1",Name=room.Name,Type=room.Theme,Clear="Explore tiles and survive.",DoorText="Reach exits."},Board=room,PlayerTile="start",PartnerTile="start",CanAct=true,ActionUsed=false,MoveOptions=nil,DoorOptions=nil,DoorVotes={},RouteWheel=nil,RoomActions={SearchUsed=false,SchemeUsed=false,SearchBy=nil,SchemeBy=nil},SearchedTiles={},EventCounter=0,ItemCounter=2,Backpack=createStartingBackpack(),PendingEquip=nil,Players={{UserId=player.UserId,Name="You",Zone="Entry",Pouch=300,Bank=0,HP=5,Keys=0,Light=0,Relics={}}, {UserId=0,Name="Partner",Zone="Entry",Pouch=300,Bank=0,HP=5,Keys=0,Light=0,Relics={}}},Enemies={},Doors=room.Doors,ActionCards=DungeonDoorsSpec.Actions,LastEvent="Board figure mode. Use command buttons, WASD for movement during Move, and E for context interact."}; setPopup(state,"Room","Dusty Entry","Room 1 / 15","One action per turn. This run gets harder."); return state end
...
local function loadNextRoomAfterDoor(player,state,chosen)
    ...
    setPopup(state,"Room",state.Board.Name,"Room "..tostring(state.RoomIndex).." / "..tostring(state.RoomsToWin),"Search is per tile. Interact only appears when this tile has a real use.")
    sendState(player,"Join")
end
...
local function chooseDoor(player,state,doorId)
    ...
    if monsterAlive then
        ...
        state.LastEvent="Door choice cancelled. "..tostring(monsterTile.Enemy or "A monster").." is still alive in the room."
        setPopup(state,"Door Locked","Monster holds the route",monsterTile.Enemy or "Monster","Clear revealed monsters before leaving.")
        return
    end
...
local function submitDungeonAction(player,actionKey)
    ...
    if actionKey==Constants.ACTIONS.Step then
        ...
        if #state.MoveOptions==0 then
            state.LastEvent=currentTileHasLiveMonster(state) and "Monster blocks the way. No revealed escape tile is adjacent." or "No reachable tile."
            setPopup(state,"Info","No Movement",state.Board.Name,state.LastEvent)
            sendState(player,"Join")
            return
        end
        state.CanAct=false
        state.ActionUsed=true
        state.Phase=currentTileHasLiveMonster(state) and "Run Away" or "Move Figure"
        state.LastEvent=currentTileHasLiveMonster(state) and "Monster nearby. Use WASD or click a revealed connected room to run." or "Use WASD or click a glowing connected room."
        sendState(player,"Join")
        return
    ...
    elseif actionKey==Constants.ACTIONS.Strike then
        if tile and tile.Kind=="Enemy" and not tile.Cleared and (tile.HP or 0)>0 then
            ...
            if tile.HP<=0 then
                ...
                state.LastEvent=(tile.Enemy or "Enemy").." defeated. Doors can open again if no other monsters remain."
                progressTask(profile,"DefeatEnemy")
                setPopup(state,"Combat","Monster defeated",tile.Enemy,"The room breathes for a second. Threat -1.")
            else
                state.LastEvent=(tile.Enemy or "Enemy").." is wounded. Enemy turn incoming."
                setPopup(state,"Combat","Hit landed",tile.Enemy,"The enemy survives. Watch its response.")
            end
        else
            state.LastEvent="No live monster on this tile."
            setPopup(state,"Info","No target","Fight has no target","Move onto or reveal an enemy tile first.")
        end
    ...
    elseif actionKey==Constants.ACTIONS.Search then
        ...
        if state.SearchedTiles[key] then
            state.LastEvent="This tile was already searched. Team-shared search means your teammate cannot search it again either."
            setPopup(state,"Info","Already searched",tile.Label,"Search is once per tile, not once per room.")
            sendState(player,"Join")
            return
        end
        if currentTileHasLiveMonster(state) then
            state.LastEvent="You cannot safely search while a monster is standing on this tile."
            setPopup(state,"Info","Too dangerous",tile.Enemy or "Monster","Fight, brace, or run first.")
            sendState(player,"Join")
            return
        end
        ...
        if tile.Kind=="Trap" and not tile.Cleared then
            state.LastEvent="Search found the trap mechanism. Interact can now disarm it."
            tile.SearchClue="Trap mechanism found"
            state.Phase="Clue Found"
            setPopup(state,"Search","Trap mechanism",tile.Label,"Interact can disarm this instead of blindly stepping through.")
        elseif tile.Kind=="Treasure" and not tile.Cleared then
            state.LastEvent="Search checked the chest. It can still reward or bite, but Interact is now informed."
            tile.SearchClue="Chest inspected"
            setPopup(state,"Search","Chest inspected",tile.Label,"Interact opens it. Risk depends on Threat.")
        elseif tile.Kind=="Discovery" and not tile.Cleared then
            state.LastEvent="Search found a room clue. Interact can turn it into a real discovery."
            tile.SearchClue="Discovery clue found"
            setPopup(state,"Search","Clue found",tile.Label,"Interact with the clue to use it.")
        else
            applyOutcome(player,state,chooseWeighted(state.Board.SearchOutcomes,Random.new(todaySeed()+state.EventCounter+state.RoomIndex+state.Threat)),"Search")
        end

    elseif actionKey==Constants.ACTIONS.Interact then
        local hasInteract, interactLabel=currentTileHasInteract(state)
        if not hasInteract then
            state.LastEvent="Nothing useful to interact with on this tile."
            setPopup(state,"Info","No interaction",tile and tile.Label or "Unknown","Move/Search first. Interact only appears when something has a use.")
            sendState(player,"Join")
            return
        end
        local monsterAlive, monsterTile=hasAnyLiveRevealedMonster(state)
        if monsterAlive and tile and tile.Kind=="Exit" then
            state.LastEvent="Exit interaction locked. "..tostring(monsterTile.Enemy or "A monster").." is alive."
            setPopup(state,"Door Locked","Monster holds the exit",monsterTile.Enemy or "Monster","Defeat monsters before leaving.")
            sendState(player,"Join")
            return
        end

        if tile.Kind=="Treasure" then
            local item=addItem(state,LOOT_TABLE[((state.RoomIndex+state.EventCounter+#state.Backpack.Items)%#LOOT_TABLE)+1])
            local threatRisk=state.Threat>=6 and " Threat is high, so the chest snaps back. Threat +1." or ""
            if state.Threat>=6 then state.Threat += 1 end
            tile.Cleared=true
            state.LastEvent="Opened chest: "..item.Data.Name.."."..threatRisk
            setPopup(state,"Treasure","Chest opened",item.Data.Name,item.Data.Body..threatRisk)
        elseif tile.Kind=="Discovery" then
            tile.Cleared=true
            state.Players[1].Keys += 1
            state.LastEvent="Discovery used. A route clue becomes real. +1 Key for now."
            setPopup(state,"Discovery","Clue used",tile.Label,"The board gives you a key/route clue.")
        elseif tile.Kind=="Trap" then
            tile.Cleared=true
            state.Threat=math.max(0,state.Threat-1)
            state.LastEvent="Trap disarmed. Threat -1."
            setPopup(state,"Interact","Trap disarmed",tile.Label,"Good interaction prevents fake puzzle damage.")
    ...
    elseif actionKey==Constants.ACTIONS.Shield then
        ...
        state.Threat=math.max(0,state.Threat-1)
        state.LastEvent="Braced. +1 HP and -1 Threat. Use this when enemy intent or Threat is scary."
        progressTask(profile,"ShieldPartner")
        setPopup(state,"Guard","Braced","Defensive stance","+1 HP. Threat reduced.")

    elseif actionKey==Constants.ACTIONS.Scheme then
        state.LastEvent="Scheme is removed as a generic action. It will return only as a named interaction when a tile/object actually needs it."
        setPopup(state,"Removed","No generic Scheme","Use Interact instead","Special tricks should be tied to real room objects.")
        sendState(player,"Join")
        return
    end

    beginNextTurn(player,state,1.35)
